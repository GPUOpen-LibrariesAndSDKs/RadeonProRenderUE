    ...
    expression(_expression)
{
    ...
    for (int i = 0; i < expression->GetInputs().Num(); ++i)
    {
        ...
        {
            nodes.emplace_back(std::make_shared<UE4InterchangeMaterialNode>(
                input->Expression));
            muxes.emplace_back(std::make_unique<UE4InterchangeMaterialNodeMux>(
                nodes.back()));

            inputNames.emplace_back(
                TCHAR_TO_ANSI(*expression->GetInputName(i))
            );
        }
    }
