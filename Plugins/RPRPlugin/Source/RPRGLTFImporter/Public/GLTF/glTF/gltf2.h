...
namespace gltf
{
    ...
    struct Buffer : glTFChildOfRootProperty
    {
		...
		bool appendable = false; // Only used during Export -  =true if this buffer can contains several sub-buffers -  see  ExportBuffer_AutoFile 
	...
	struct RprBuffer : glTFChildOfRootProperty
    {
		int data_bufferIndex = 0;
		unsigned long long data_bufferOffset = 0;
		uint64_t data_bufferSize = 0;
		uint32_t desc_nb_element = 0;
		uint32_t desc_element_type = 0;
		uint32_t desc_element_channel_size = 0;
    };
    ...
    struct glTF : glTFProperty
    {
		...
		std::vector<RprBuffer> rprBuffers; // An array of rprBuffers.
