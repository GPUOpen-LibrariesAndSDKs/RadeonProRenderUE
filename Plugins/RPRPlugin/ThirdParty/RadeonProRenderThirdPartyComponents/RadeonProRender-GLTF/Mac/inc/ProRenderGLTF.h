...
#define RPRGLTF_EXPORTFLAG_GLB (1 << 0) // if flag enabled, export to a binary GLTF
...
*/
extern int rprExportToGLTF(const char* filename, rpr_context context, rpr_material_system materialSystem_NOT_USED, rprx_context uberMatContext, const rpr_scene* scenes, size_t sceneCount, unsigned int exportFlags);
...
extern int rprGLTF_DeleteListImportedObjects(rprx_context contextX);
